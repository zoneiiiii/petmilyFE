...
const VolunteerNotice = () => {
  ...
  const handleChange = (event, value) => { //페이지 변경 시 호출, 새 페이지의 번호를 value에 저장함.
    setPage(value); 
  };
  ...
  return (
    <S.Container>
      ...
      <S.CardGrid>
        ...
        {cards.map((card) => ( //map함수로 cards에 있는 데이터 전부 보여줌.
          <VolunteerCard {...card} key={card.boardNum} />
        ))}
      </S.CardGrid>
      </S.CardContainer>
      <VolunteerPagination count={pageCount} page={page} onChange={handleChange} />
    </S.Container>
  );
};
